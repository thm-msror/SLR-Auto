...
|          | warplane                                              |  |  |  |
| natural  | disaster,<br>earthquake,<br>fire,<br>flame,<br>flood, |  |  |  |
| disaster | hurricane, tornado, tsunami                           |  |  |  |
...
```
Let S_1 = senses(term<sub>1</sub>)

Let S_2 = senses(term<sub>2</sub>)

for all (s_1, s_2) where s_1 \chi S_1, s_2 \chi S_2

maxlesk = max(lesk(s_1, s_2)) }
```
...
C = set of concepts from fixed lexicon
                                               Input
CS = \text{set of synonyms } S \text{ for all concepts in } C
                                               Input
QC = set of concept confidences for query Q
                                               Output
for each QT_i in QT
  for each C_i in C
     for each C_jS_k in C_jS
                                               Step 1
       conf_k = maxlesk(C_jS_k, QT_i) }
     QC_i += \max(conf_k) }
                                               Step 2
for each C_i in C
                                               Step 3
...
for each S<sub>i</sub> in S

for each C<sub>j</sub> in C

S<sub>i</sub>S += QC<sub>i</sub> * SC<sub>i</sub> } }
```
...
**Figure 3.** Performance evaluation (y-axis = Average Precision at 1000) of text-based search, model-based search, and model-based reranking on 24 TRECVID 2005 topics. Model-based re-ranking performed by simple averaging fusion of text-based and model-based retrieval results after score range normalization.
...
<sup>&</sup>lt;sup>2</sup> http://www-nlpir.nist.gov/projects/tv2005/tv2005.html
